...
def GenerateLabeledPaches(patch_dir, sha):
    output_dir = os.path.join(patch_dir, 'chromium')
    output_dir2 = os.path.join(patch_dir, 'chromium2')
    os.makedirs(output_dir)
    os.makedirs(output_dir2)
    data = {'path': '.'}
    with open(os.path.join(output_dir, 'chromium.meta'), 'w') as f:
        json.dump(data, f)
    with open(os.path.join(output_dir2, 'chromium2.meta'), 'w') as f:
        json.dump(data, f)
...
def main(argv=None):
    ...
    for repo in repos:
        ...
        if result.returncode != 0:
            logging.warning("Failed on finding dependency of %s: %s", repo, result.stderr)
            continue
