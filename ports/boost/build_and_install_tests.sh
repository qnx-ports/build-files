...
copy_test_binaries() {
    test_folder_path="./build-files/ports/boost/$1/build/boost/bin.v2/boost_tests"
    mkdir -p $test_folder_path

    # Find test binaries and copy to $test_folder_path
    for lib in "./build-files/ports/boost/$1/build/boost/bin.v2/libs"/* ; do
    ...
    for f in $(find ./build-files/ports/boost/$1/build/boost/bin.v2/libs -type f -name *.so*);
    do
...
for lib in "./build-files/ports/boost/nto-x86_64-o/build/boost/bin.v2/libs"/* ; do
    QNX_PROJECT_ROOT="$(pwd)/boost" make -C build-files/ports/boost/ test."$(basename $lib)" -i -j$(nproc)
done
